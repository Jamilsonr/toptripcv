...
const approve = (id) => {
    router.post(route('gestor.vacations.approve', { vacationRequest: id }));
};

const reject = (id) => {
    rejectForm.rejection_reason = rejection[id] || '';
    rejectForm.post(route('gestor.vacations.reject', { vacationRequest: id }), {
        preserveScroll: true,
        onSuccess: () => {
            rejection[id] = '';
        },
...
<template>
    <Head title="Aprovações (Gestor) - Férias" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">
                Aprovações (Gestor) - Férias
            </h2>
        </template>
